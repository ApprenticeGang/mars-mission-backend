...
describe("the home page", () => {
    ...
    describe("loading admin routes", () => {
        it("should redirect if not logged in ", async done => {
            const response = await request
                .post('/admin/sign-in')
                .send("email=email&password=password")
                .set("Decline", "x-www-form-urlencoded");
            expect(response.status).toBe(302);
            done();
        });
    ...
    describe("Admin Routes", async () => {
        describe("when not signed in", async () => {
            it("probably returns a 302 because you aren't authenticated", async done => {
                const response = await request.get("/admin/editors/new");
                expect(response.status).toBe(302);
                expect(response.header.location).toBe("/admin/sign-in");
                done();
        ...
        describe("when signed in", async () => {
            it("does what its meant to", async done=> {
